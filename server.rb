...
def empty_chart_response(ticker, range, interval, message = nil)
  {
    ...
    "points" => [],
    "unavailable" => true,
    "message" => message || "Chart data is temporarily unavailable."
  }
end
...
def finnhub_chart(ticker, range)
  ...
  warn "Finnhub chart unavailable: #{e.message}"
  empty_chart_response(
    ticker,
    range,
    resolution || chart_interval_for_range(range),
    "Price history charts require a Finnhub premium plan. Quote, metrics, and other research data are still shown below."
  )
end
...
def fetch_chart(ticker, range, interval)
  ...
  message = if e.message.include?("429")
              "Yahoo Finance is rate-limiting this server. Chart data is unavailable right now."
            else
              e.message
            end
  empty_chart_response(ticker, range, interval, message)
end
...
def build_research_profile(ticker)
  ...
  build_research_profile_with_yahoo_fallback(symbol)
end

def build_research_profile_with_yahoo_fallback(symbol)
