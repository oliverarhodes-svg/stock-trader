...
const POPULAR_TICKERS = [
  ...
  { symbol: "PG", name: "Procter & Gamble Co.", type: "EQUITY" },
  ...
  { symbol: "COST", name: "Costco Wholesale Corp.", type: "EQUITY" },
...
function filterLocalTickers(query) {
...
function hideResearchError() {
  ...
  researchError.classList.remove("warning-banner");
  researchError.classList.add("error-banner");
}

function showResearchError(message, isWarning = false) {
  researchError.textContent = message;
  researchError.classList.remove("hidden");
  researchError.classList.toggle("warning-banner", isWarning);
  researchError.classList.toggle("error-banner", !isWarning);
}
...
researchQuery.addEventListener("input", () => {
  ...
  const localMatches = filterLocalTickers(query);
  ...
  searchTimeout = setTimeout(async () => {
    ...
    const currentQuery = researchQuery.value.trim();
    ...
    try {
      ...
      renderSuggestions(mergeSuggestions(filterLocalTickers(query), data.results || []));
    } catch {
      renderSuggestions(filterLocalTickers(query));
    }
  }, 450);
});

researchQuery.addEventListener("focus", () => {
  const query = researchQuery.value.trim();
  if (query.length > 0) {
    renderSuggestions(filterLocalTickers(query));
  }
...
function hideChartUnavailable() {
  document.getElementById("chart-unavailable").classList.add("hidden");
...
async function loadChart(ticker, range) {
  ...
  const data = await response.json();
  if (!response.ok) {
    showChartUnavailable(data.detail || "Chart unavailable.");
    return;
  }

  if (!data.points || data.points.length === 0) {
    showChartUnavailable(data.message || "No chart data for this range.");
    return;
  }

  hideChartUnavailable();
...
function renderResearch(data) {
  ...
  if (data.partial_data && data.warning) {
    showResearchError(`Limited data: ${data.warning}. Showing available price and chart info.`, true);
  } else {
    hideResearchError();
  }
...
async function loadResearch(ticker) {
  ...
  try {
    ...
    if (!response.ok) {
      const message = data.detail || "Research failed";
      if (message.includes("429")) {
        throw new Error("Yahoo Finance is rate-limiting requests. Wait 30 seconds and try again.");
      }
