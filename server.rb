...
def finnhub_http_get(path, params = {}, cache_ttl: 120)
  ...
  raise "Finnhub returned #{response.code}" unless response.is_a?(Net::HTTPSuccess)
...
def build_research_profile_finnhub(ticker)
  ...
  profile = finnhub_http_get("/stock/profile2", { symbol: symbol }, cache_ttl: 3600)
  metrics = finnhub_http_get("/stock/metric", { symbol: symbol, metric: "all" }, cache_ttl: 3600)
  recommendations = finnhub_http_get("/stock/recommendation", { symbol: symbol }, cache_ttl: 3600)
  earnings = finnhub_http_get("/stock/earnings", { symbol: symbol }, cache_ttl: 3600)

  is_etf = false
  etf_profile = nil
  begin
    etf_profile = finnhub_http_get("/etf/profile", { symbol: symbol }, cache_ttl: 3600)
    is_etf = true
  ...
  if is_etf
    begin
      etf_profile ||= finnhub_http_get("/etf/profile", { symbol: symbol }, cache_ttl: 3600)
      etf_holdings = finnhub_http_get("/etf/holdings", { symbol: symbol }, cache_ttl: 3600)
      ...
      warn "Finnhub ETF data failed for #{symbol}: #{e.message}"
    end
  end
...
def yahoo_chart(ticker, range, interval)
  return finnhub_chart(ticker, range) if finnhub_configured?
