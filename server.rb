...
def yahoo_http_get(url, cookie: nil)
  uri = URI(url)
...
def yahoo_session
  cookie_response = Net::HTTP.start("fc.yahoo.com", 443, use_ssl: true, open_timeout: 10, read_timeout: 10) do |http|
    req = Net::HTTP::Get.new("/")
    req["User-Agent"] = USER_AGENT
    http.request(req)
  end
  cookie = cookie_response["Set-Cookie"].split(";").first

  crumb = Net::HTTP.start("query1.finance.yahoo.com", 443, use_ssl: true, open_timeout: 10, read_timeout: 10) do |http|
    req = Net::HTTP::Get.new("/v1/test/getcrumb")
    req["User-Agent"] = USER_AGENT
    req["Cookie"] = cookie
    http.request(req)
  end
  raise "Could not fetch Yahoo session" unless crumb.is_a?(Net::HTTPSuccess)

  { cookie: cookie, crumb: crumb.body.strip }
end

def yahoo_quote_summary(ticker, modules)
  session = yahoo_session
  module_list = modules.join(",")
  url = "https://query1.finance.yahoo.com/v10/finance/quoteSummary/#{ticker}?modules=#{module_list}&crumb=#{session[:crumb]}"
  data = yahoo_http_get(url, cookie: session[:cookie])
  result = data.dig("quoteSummary", "result", 0)
  raise "No data found for #{ticker}" if result.nil?
...
def yahoo_search(query)
  ...
  data = yahoo_http_get("https://query2.finance.yahoo.com/v1/finance/search?q=#{encoded}&quotesCount=8&newsCount=0")
  quotes = data["quotes"] || []
...
def fetch_current_price(ticker)
  symbol = ticker.upcase.strip
  data = yahoo_http_get("https://query1.finance.yahoo.com/v8/finance/chart/#{symbol}?interval=1d&range=1d")
...
server.mount_proc "/api/research/chart" do |req, res|
  ...
  begin
    json_response(res, 200, yahoo_chart(ticker, range, interval))
  rescue StandardError => e
    json_response(res, 422, { "detail" => e.message })
  end
end
