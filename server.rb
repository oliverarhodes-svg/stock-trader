...
def yahoo_http_get(url, cookie: nil, cache_ttl: nil)
  ...
  4.times do |attempt|
    ...
    sleep(1.5 * (attempt + 1) + rand * 0.5) if last_code == 429 && attempt < 3
  end
...
def fetch_yahoo_crumb(cookie)
  [0, 1, 2].each do |attempt|
    ...
    sleep(1 + attempt) if response.code.to_i == 429
  end
...
def yahoo_quote_summary(ticker, modules)
  ...
  2.times do |attempt|
    ...
    data = yahoo_http_get(url, cookie: session[:cookie], cache_ttl: 300)
...
def yahoo_search(query)
  ...
  last_error = nil
  urls.each do |url|
    data = yahoo_http_get(url, cache_ttl: 600)
    quotes = data["quotes"] || []
  ...
  raise last_error || "Search unavailable"
end
...
def yahoo_chart(ticker, range, interval)
  ...
  data = yahoo_http_get(url, cache_ttl: 120)
...
def build_research_profile(ticker)
  ...
  begin
    ...
    raise "Yahoo Finance is rate-limiting requests. Wait 30 seconds and try again."
  end
end

def yahoo_chart_meta(ticker)
  data = yahoo_http_get("https://query1.finance.yahoo.com/v8/finance/chart/#{ticker}?range=1d&interval=1d&includePrePost=false", cache_ttl: 60)
